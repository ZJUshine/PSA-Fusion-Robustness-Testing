...
def extract_frustum_data(idx_filename, split, output_filename, viz=False,
    ...
    '''
    dataset = kitti_object(os.path.join(ROOT_DIR,'dataset/lidar_laser_creating_car/object'), split)
...
def get_box3d_dim_statistics(idx_filename):
    ...
    dataset = kitti_object(os.path.join(ROOT_DIR,'dataset/lidar_laser_creating_car/object'))
    dimension_list = []
    type_list = []
    ry_list = []
    data_idx_list = [int(line.rstrip()) for line in open(idx_filename)]
...
if __name__=='__main__':
    ...
    if args.car_only:
        type_whitelist = ['Car']
        output_prefix = 'lidar_laser_creating_car_frustum_caronly_'
    else:
    ...
    if args.gen_train:
        extract_frustum_data(\
            ...
            'training',
            os.path.join(BASE_DIR+'/lidar_laser_creating_car', output_prefix+'train.pickle'), 
    ...
    if args.gen_val:
        extract_frustum_data(\
            ...
            'training',
            os.path.join(BASE_DIR+'/lidar_laser_creating_car', output_prefix+'val.pickle'), 
            viz=False, perturb_box2d=False, augmentX=1,
            type_whitelist=type_whitelist)

    if args.gen_val_rgb_detection:
        extract_frustum_data_rgb_detection(\
            os.path.join('/home/usslab/Documents3/xuancun/sensorfusion-magician/frustum-pointnets/kitti/', 'rgb_detections_kitti/rgb_detection_val.txt'), 
            'training',
            os.path.join(BASE_DIR+'/lidar_laser_creating_car', output_prefix+'val_rgb_detection.pickle'), 
            viz=False,
            type_whitelist=type_whitelist)
